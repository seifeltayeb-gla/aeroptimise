function [err] = err_p(p_u, p_k, x_f, z_f)
%
%err_p calculates discrency between calculated and imported aerofoil coordinates
...
%% calculates parmetrised aerofoil coordinates
    zc_u = zeros(length(x_f),1); zc_l = zc_u; % creates empty arrays for z-coordinates

    for i = 1:6 %calculates point z-coordinates
    zc_u = zc_u + a_up(i)*x_f.^(i-0.5); zc_l = zc_l + a_lo(i)*x_f.^(i-0.5);
    end

    z_c=vertcat(flipud(zc_u(2:end)),zc_l); % concenates z-coordiantes (TE-LE-TE)
%% calculates MS error
    err = mean((z_c-z_f).^2);
end
