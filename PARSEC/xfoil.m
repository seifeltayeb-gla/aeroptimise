...
% Some default values
if ~exist('coord','var'), coord = 'seligFmt_coord/e332.dat'; end;
...
if (fid<=0),
  ...
  for ii = 1:Nalpha
    ...
    file_dump{ii} = sprintf('%s_a%06.3f_%.8i_dump.dat',fname,now,alpha(ii));
    file_cpwr{ii} = sprintf('%s_a%06.3f_%.8i_cpwr.dat',fname,now,alpha(ii));
    % Commands
    fprintf(fid,'alfa %g\n',alpha(ii));
    fprintf(fid,'dump %s\n',file_dump{ii});
    fprintf(fid,'cpwr %s\n',file_cpwr{ii});
  ...
  % executes XFOIL based on platform
  if ispc
  cmd = sprintf('cd %s && xfoil_win.exe < xfoil.inp > xfoil.out',wd);
  elseif isunix
  cmd = sprintf('cd %s && ./xfoil_unix < xfoil.inp > xfoil.out',wd);
  elseif ismac
    cmd = sprintf('cd %s && ./xfoil_mac < xfoil.inp > xfoil.out',wd);
  end
