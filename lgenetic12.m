...
pfNAME=sprintf('plogs/%s_%s_lift12.txt',n,datestr(now,'mmddHHMM')); 
...
fprintf('Optimisation started on %.2i/%.2i/%.4i at %.2i:%.2i:%.2i',fliplr(sT(1:3)),sT(4:5),round(sT(6)))
    ...
    del_l = @lopt; %objective function, RMS difference between original
...
fprintf(pfID,'|---P1----|----P2---|----P3---|----P4---|----P5---|----P6---|----P7---|----P8---|----P9---|---P10---|---P11---|---P12---|----CL---|---RMS---|\n');
fprintf(pfID,'|-------------------------------------------------------------------------------------------------------------------------------------------|');
fprintf(pfID,'\n|%+5f|%+5f|%+5f|%+5f|%+5f|%+5f|%+3f|%+5f|%+5f|%+5f|%+5f|%+5f|%+5f|%+5f|',P(1),P(2),P(3),P(4),P(5),P(6),P(7),P(8),P(9),P(10),P(11),P(12),CL,0);
fclose(pfID);
%% GENETIC ALGORITHM SETUP
...
[x,fval,exitflag,output,population] = ga(del_l,nvars,[],[],[],[],LB,UB,[],[],options);
